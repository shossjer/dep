...
function(_get_dep_config_type outvar config)
	get_property(_nondebug DIRECTORY PROPERTY DEP_NONDEBUG_CONFIG)
	get_property(_debug DIRECTORY PROPERTY DEP_DEBUG_CONFIG)
	if(NOT "${_nondebug}")
		set(_nondebug Release)
	...
	if("${config}" STREQUAL "${_nondebug}")
		set(${outvar} optimized PARENT_SCOPE)
	elseif("${config}" STREQUAL "${_debug}")
		set(${outvar} debug PARENT_SCOPE)
	else()
		message(FATAL_ERROR "Unexpected config")
	endif()
endfunction()
...
function(dep_package name target)
	...
	foreach(_config IN ITEMS ${_configs})
		...
		if("${_built}" OR "${_parsed_IGNORE_SYSTEM}")
			set(_file_path "${_dep_directory}/${_config}/include")
		...
		_get_dep_config_type(_config_type ${_config})
		set(_types optimized debug)

		set(_found_all_files True)
		set(_file_directory "")
		set(_file_type "")
		foreach(_file IN ITEMS ${_parsed_FILES})
			if("${_file_directory}" STREQUAL "IN_DIRECTORY")
				set(_file_directory "/${_file}")
			elseif("${_file}" STREQUAL "IN_DIRECTORY")
				set(_file_directory "IN_DIRECTORY")
			elseif(_file IN_LIST _types)
				if(_file_type)
					message(WARNING "Unexpected keyword: ${_file}, expected a file after ${_file_type}")
				endif()

				set(_file_type ${_file})
			elseif("${_file_type}" STREQUAL "" OR "${_file_type}" STREQUAL "${_config_type}")
				string(TOUPPER "${_file}" _big_file)
				string(MAKE_C_IDENTIFIER "${_big_file}" _c_file)

				# note we would have liked to be able to use PATH_SUFFIXES
				# for the IN_DIRECTORY specification but it appears as
				# though NO_DEFAULT_PATH ignores that option
				if(_file_path)
					find_file(FILE_${_c_file}_${_big_config} "${_file}" PATHS "${_file_path}/${_file_directory}" NO_DEFAULT_PATH)
				elseif(_file_directory)
					find_file(FILE_${_c_file}_${_big_config} "${_file}" PATH_SUFFIXES ${_file_directory})
				else()
					find_file(FILE_${_c_file}_${_big_config} "${_file}")
				endif()
		...
		endforeach()
		if("${_file_directory}" STREQUAL "PATH_SUFFIXES")
			message(FATAL_ERROR "Expected one more directory after PATH_SUFFIXES")
		endif()
		if(_file_type)
			message(FATAL_ERROR "Expected one more file after ${_file_type}")
		endif()
		...
		if(_parsed_LIBS)
			...
			foreach(_lib IN ITEMS ${_parsed_LIBS})
				if(_lib IN_LIST _types)
					if(_lib_type)
						message(WARNING "Unexpected keyword: ${_lib}, expected a lib after ${_lib_type}")
					endif()

					set(_lib_type ${_lib})
				elseif("${_lib_type}" STREQUAL "" OR "${_lib_type}" STREQUAL "${_config_type}")
			...
			endforeach()
			if(_lib_type)
				message(FATAL_ERROR "Expected one more lib after ${_lib_type}")
			endif()
